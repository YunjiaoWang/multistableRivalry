...
{ 
  ...
  dataList = list(
    ...
    s = s,
    Ntotal = Ntotal ,
    Nsubj = Nsubj  # should equal length(unique(s))
  )
  ...
  model {
  for ( i in 1:Ntotal ) {
  y[i] ~ dt(  beta0[s[i]] + beta1[s[i]] * x[i], 1/sigma^2, 4 )
  }
  for ( s in 1:Nsubj ) {
      beta0[s] ~ dnorm( beta0mu , 1/(beta0sigma)^2 )  
      beta1[s] ~ dnorm( beta1mu , 1/(beta1sigma)^2 )
    }
  # Priors vague 
  beta0mu ~ dnorm( 0 , 1/(10)^2 )
  beta1mu ~ dnorm( 0 , 1/(10)^2 )
  sigma ~ dunif( 1.0E-3 , 1.0E+3 )
  beta0sigma ~ dunif( 1.0E-3 , 1.0E+2 )
  beta1sigma ~ dunif( 1.0E-3 , 1.0E+2 )
  }
  ...
  #-----------------------------------------------------------------------------
  # RUN THE CHAINS
  parameters = c( "beta0" ,  "beta1" , "beta0mu" , "beta1mu" , "beta0sigma" , "beta1sigma" )
