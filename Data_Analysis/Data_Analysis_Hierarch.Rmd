...
```{r single_green_compare, results='asis',echo=FALSE}
library(ggplot2)
outputSingle <-output.single
outputGreen <- output.green
outputSingle <- cbind(outputSingle, perc = c(rep('single',length(outputSingle$mean))))
outputGreen <- cbind(outputGreen, perc = c(rep('green',length(outputGreen$mean))))
outputAll <- rbind(outputSingle, outputGreen)
outputAll$perc <- as.factor(outputAll$perc)
outputAll$ID <- factor(outputAll$ID, levels=ID_order1)

ggplot(outputAll, aes(x=ID,y=mean,fill=perc))+geom_bar(stat="identity",position=position_dodge(width=0.85))+geom_errorbar(aes(ymin=lowCI, ymax=highCI), width=.1, position=position_dodge(width=0.85))+
  theme(text = element_text(size=20), axis.text = element_text(size=16))+
        #panel.grid.major = element_line(colour = "grey95"),
        #panel.background=element_rect(fill="white"))+
  xlab("Subject")+
...
```{r single_green_compare2, results='asis',echo=FALSE}
library(ggplot2)
outputSingle <-output.single
outputGreen <- output.green
outputSingle <- cbind(outputSingle, perc = c(rep('single',length(outputSingle$mean))))
...
for (subject in levels(data$IDs)){
  ...
  for (i in c(0.4,0.9)){
    ...
    #plot histogram
    hist(toplot, breaks=22, freq = FALSE, col = "blue",xlab=paste("Duration [s], mean=", round(m,2),"sd=",round(sdw,2)), 
